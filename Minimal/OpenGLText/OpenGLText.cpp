...
//Shader * shader_text; // text shader
//const char * TEXT_VERT = "shader_text.vert";
//const char * TEXT_FRAG = "shader_text.frag";
...
{
	// shaders
	//shader_text = new Shader(TEXT_VERT, TEXT_FRAG);
...
bool OpenGLText::init(const char * fontName, int w, int h)
{
    ...
    if(fontName)
    {
        ...
        if(fd)
        {
			if (glyphInfos != nullptr && allocated == true) {
				delete glyphInfos;
			}
			//cout << "hello" << endl;
...
bool OpenGLText::init(int w, int h)
{
    ...
    if (m_widgetProgram == 0)
    {
		...
		//cout << "both shaders compiled" << endl;
        //CHECKGLERRORS();
...
void OpenGLText::endString()
{
    if(!m_vertices.empty())
    {
        ...
        //
        glUseProgram( m_widgetProgram/*shader_text->ID*/ );
        glProgramUniform4f( m_widgetProgram/*shader_text->ID*/, m_canvasVar, m_canvas.w, m_canvas.h, m_canvas.ratio, 0 );
        //
        // Draw
        //
#ifdef USE_PSEUDO_INSTANCING
...
{
	if (!glyphInfos) {
		cout << "glyphInfos is null" << endl;
		return;
	}
	else {
		//cout << "glyphInfos isn't null" << endl;
	}
...
float OpenGLText::drawString( int x, int y, const char * text, int nbLines, unsigned long color)
{
	...
	//cout << "color4f: " << color4f[0] << ", " << color4f[1] << ", " << color4f[2] << ", " << color4f[3] << endl;
...
{ 
	...
	if (glyphInfos == nullptr) {
		//cout << "glyphInfos is null" << endl;
		return 0;
	}

	//cout << "glyphInfos->pix.ascent: " << glyphInfos->pix.ascent << endl;
	//cout << "glyphInfos->pix.descent: " << glyphInfos->pix.descent << endl;
	//cout << "glyphInfos->pix.linegap: " << glyphInfos->pix.linegap << endl;
    ...
    glProgramUniform4fv(m_widgetProgram/*shader_text->ID*/, m_color, 1, color4f);
    ...
    while (*c != '\0')
    {
        ...
        if ( *c == '\n')
        {
			...
			cout << "lPosX: " << lPosX << ", lPosY: " << lPosY << endl;
        }
        else if ( *c > 128 || *c < 0 )
        {
			cout << "else if " << endl;
        ...
        else
        {
			...
			//cout << "usedHeight: " << usedHeight << endl;
			//cout << "c: " << c << endl; // prints correctly
